...
func bootAdminWebService() {
    ...
    router.get("/json") {
        ...
        let data :[String:String] = ["framework":"Ig2SQLWebAdminService",
    ...
    srv.started {
        ...
        Log.info("--Server \(serverip)   WebAdmin Service started on port \(Config.webadmin_port)")
    }
    srv.failed {status in
        Log.error("--Server \(serverip)   WebAdmin Service FAILED TO START   status \(status)   ")
        exit(1)
    }
}
